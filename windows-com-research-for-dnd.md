...
Since `syscall.NewCallback` uses the common `runtime.cgocallback_gofunc` procedure, it will create a new "m" for your callback if started on a new thread—but only if CGO is enabled.
...
**What we know**:
- Windigo's documentation states it's "100% pure Go – CGo is not used, just native syscalls"
...
```
┌──────────────────────────────────────────────────────────┐
│ DROPFILES Structure (20 bytes on x86, 24 on x64)        │
├──────────────────────────────────────────────────────────┤
│ pFiles: DWORD    - Offset to file list from start       │
│ pt: POINT        - Drop point (x, y)                    │
│ fNC: BOOL        - Non-client area flag                 │
│ fWide: BOOL      - TRUE if Unicode strings              │
├──────────────────────────────────────────────────────────┤
│ File 1: "C:\path\to\file1.txt\0"  (null-terminated)     │
├──────────────────────────────────────────────────────────┤
│ File 2: "C:\path\to\file2.txt\0"  (null-terminated)     │
├──────────────────────────────────────────────────────────┤
│ ... more files ...                                       │
├──────────────────────────────────────────────────────────┤
