...
*** Architecture Rationale:
    Config.json and SQLite serve DIFFERENT purposes - keep them distinct:
    - config.json = User-editable settings, UI composition, AND favorites
    - SQLite = Application-managed dynamic data (search history only for now)

    Data flow:
    - config.json is THE source of truth for all settings and favorites
    - Load config.json into memory on startup (Config struct)
    - Application reads from in-memory Config struct (fast, no I/O)
    - Settings UI writes to config.json, then reloads into memory
    - SQLite is ONLY for search history (append-only, queryable data)

    Why favorites belong in config.json:
    - User-editable: power users can manually edit favorites
    - Portable: copy config.json to new machine = all settings + favorites
    - Simple: no DB schema migrations for favorites changes
    - Grouped favorites are just nested JSON (natural fit)
...
*** Keep in SQLite (store.DB):
    - Search history (append-only, searchable, limitable)
    - Recent files/directories (queryable by date, path)
    - File tags/labels (many-to-many relationships)
    - Cached metadata (thumbnails, directory sizes)
...
** TODO PRIORITY 3: Search History (SQLite)
   Implement search history using SQLite for queryable, append-only data.

*** Features:
    - Store past search queries with timestamps
    - Quick access to recent/frequent searches
    - Auto-complete dropdown from search history
    - Limit history size (configurable in config.json)
    - Clear history option in settings

*** Implementation:
    - Table: search_history (id, query, timestamp, result_count)
    - Query recent: SELECT * FROM search_history ORDER BY timestamp DESC LIMIT N
    - Query frequent: SELECT query, COUNT(*) FROM search_history GROUP BY query ORDER BY COUNT(*) DESC
    - Auto-complete: Match prefix against stored queries
    - Prune old entries periodically (keep last N or last 30 days)

** DONE PRIORITY 4: Recent Files/Directories (SQLite)
   CLOSED: [2024-11-30]
   Track recently accessed paths for quick navigation.

*** Features:
    - [X] Track recently opened files and directories
    - [X] "Recent Files" virtual folder in sidebar (above Favorites)
    - [X] Last 50 files tracked and displayed
    - [X] "Open File Location" context menu option
    - [X] Stale files filtered out automatically

*** Implementation:
    - Table: recent_files (id, path, name, is_dir, timestamp)
    - Query: SELECT * FROM recent_files ORDER BY timestamp DESC LIMIT 50
    - Virtual directory view with navigation back to real directories

** TODO PRIORITY 5: Syntax Highlighting Module (internal/highlight)
   Reusable code formatting module for preview pane and markdown code blocks.
   Uses popular open-source libraries for AST parsing and syntax highlighting.
...
*** Architecture:
    ...
    // Highlighter formats source code into richtext spans
    type Highlighter struct {
        theme   Theme           // Color scheme
        shaper  *text.Shaper    // For richtext rendering
    }

    // Highlight parses source and returns styled spans
    func (h *Highlighter) Highlight(source []byte, lang string) []richtext.SpanStyle

    // SupportedLanguages returns available language identifiers
    func (h *Highlighter) SupportedLanguages() []string
    #+END_SRC

*** Libraries to evaluate:
    - github.com/alecthomas/chroma - Full-featured syntax highlighter
    - github.com/yuin/goldmark - Markdown parsing (already planned)
    - Tree-sitter bindings (if available for Go)

*** Features:
    - [ ] Language detection from file extension
    - [ ] Language detection from markdown fence (```go, ```python, etc.)
    - [ ] JSON formatting with indentation (already have basic version)
    - [ ] Go, Python, JavaScript, TypeScript, Rust, C/C++, Shell
    - [ ] YAML, TOML, XML, HTML, CSS
    - [ ] Line numbers option
    - [ ] Theme support (light/dark modes)

*** Integration Points:
    - Preview pane: layoutPreviewPane() uses Highlighter for code files
    - Markdown preview: AST walker uses Highlighter for CodeBlock/FencedCodeBlock nodes
    - Both share same color theme based on app theme setting

** TODO PRIORITY 6: Markdown Preview with Tabs
   Add rendered markdown preview alongside raw view for .md files.

*** Features:
    - [ ] "Raw" and "Preview" tabs in preview pane header
    - [ ] Parse markdown with goldmark (github.com/yuin/goldmark)
    - [ ] Walk AST to build richtext spans
    - [ ] Support: headings, bold, italic, code, lists, links, blockquotes
    - [ ] Code blocks use Syntax Highlighting Module (Priority 5)

*** Implementation:
    - Add previewMode field to Renderer ("raw" | "rendered")
    - Add tab buttons in preview header
    - layoutMarkdownPreview() function for rendered view
    - astToRichtext() walks goldmark AST, builds []richtext.SpanStyle

** TODO PRIORITY 8: File Tags/Labels (SQLite)
   User-defined tags for organizing files and folders.

*** Features:
    - Create custom tags with colors
    - Apply multiple tags to files/folders
    - Filter/search by tags
    - Tag management UI

*** Implementation:
    - Table: tags (id, name, color)
    - Table: file_tags (id, path, tag_id) - many-to-many
    - Query by tag: SELECT path FROM file_tags WHERE tag_id = ?

** TODO PRIORITY 9: Cached Metadata (SQLite)
   Cache expensive-to-compute metadata for performance.

*** Features:
    - Thumbnail cache references (path to cached thumbnail)
    - Directory size cache (for large directories)
    - File preview cache metadata

*** Implementation:
    - Table: thumbnail_cache (id, path, thumb_path, mtime, size)
    - Table: dir_size_cache (id, path, size_bytes, file_count, cached_at)
    - Invalidate on mtime change

** TODO PRIORITY 10: Refactor orchestrator.go into logical sub-files
