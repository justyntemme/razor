...
** TODO PRIORITY 4: Recent Files/Directories (SQLite)
   Track recently accessed paths for quick navigation.

*** Features:
    - Track recently opened files and directories
    - "Recent" section in sidebar or quick-access menu
    - Configurable history length (in config.json)
    - Clear history option

*** Implementation:
    - Table: recent_paths (id, path, type, timestamp, access_count)
    - Query: SELECT * FROM recent_paths ORDER BY timestamp DESC LIMIT N

** TODO PRIORITY 5: File Tags/Labels (SQLite)
...
** TODO PRIORITY 6: Cached Metadata (SQLite)
...
** TODO PRIORITY 7: Refactor orchestrator.go into logical sub-files
