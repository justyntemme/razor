...
* Next
review this library and compare how it does gio development and what lessons/refactors we could learn form it https://github.com/oligo/gioview/blob/main/example/basic/file-explorer.go
review this PR to see how gio implemented external drag and drop so we can support it https://github.com/gioui/gio/pull/111
  icons for known filetypes (use whatever icon generation was used in recent files, assume we have a font that supports icons, except windows we may have an issue there)
...
*** DONE Multi-Select Improvements
    CLOSED: [2024-12-01]
    Priority: CRITICAL
    - [X] Shift+click for range selection
    - [X] Ctrl/Cmd+click for toggle selection
    - [X] Ctrl/Cmd+A to select all
    - [X] Click empty space to deselect all
    - [X] Selection count in status bar

*** DONE Breadcrumb Path Bar
    CLOSED: [2024-12-01]
    Priority: HIGH
    - [X] Display path as clickable segments (/ > Users > name > Documents)
    - [X] Click any segment to navigate to that directory
    - [X] Overflow handling for long paths (collapse middle with "...")
    - [X] Current directory shown in bold, parent dirs as clickable links
    - [X] Double-click breadcrumb to enter manual path edit mode
    - [ ] Right-click segment for context menu (copy path, open in new tab) - future
...
*** DONE Trash/Recycle Bin Integration
    CLOSED: [2024-12-01]
    Priority: HIGH
    - [X] Delete moves to trash instead of permanent delete
    - [X] Shift+Delete for permanent delete (bypass trash)
    - [X] Platform-specific trash locations:
      - macOS: ~/.Trash (via Finder/osascript - no restore API available)
      - Linux: ~/.local/share/Trash (freedesktop spec with .trashinfo for restore)
      - Windows: Recycle Bin API (supports restore)
    - [ ] "Empty Trash" option (not yet implemented)
    - [ ] Restore from trash UI (Windows and Linux only - macOS lacks restore API)

*** DONE Column Headers and Sorting
    CLOSED: [2024-12-01]
    Priority: MEDIUM
    - [X] Clickable column headers (Name, Size, Date, Type)
    - [X] Sort indicator (arrow up/down)
    - [X] Resizable columns by dragging dividers
    - [ ] Column reordering by dragging headers - future
...
*** DONE Keyboard Navigation
    ...
    - [X] Enter to open file/folder
    - [X] Type-to-select (type filename to jump to it)
    - [X] Tab to cycle focus between panels
    - [ ] Backspace to go up one level (in progress - conflicts with path editing)
    - [ ] Space bar for Quick Look preview (future)
...
*** DONE Grid/Icon View
    ...
    - [X] Thumbnail previews for images
    - [X] Image thumbnail caching for performance
    - [ ] Configurable icon/thumbnail size (not yet configurable)
    - [ ] View menu or toolbar toggle (not yet in menu)
...
*** TODO Terminal Integration
    - [X] "Open Terminal Here" context menu (right-click on folder or empty space)
    - [ ] Embedded terminal panel (toggle)
    - [ ] Configurable terminal app

*** TODO Git Integration
    Priority: HIGH (Differentiator)
    A major differentiator from Explorer/Finder - built-in git awareness.

**** File Status Icons
     - [ ] Show git status icons next to files (modified, staged, untracked, ignored)
     - [ ] Color coding: green (staged), yellow (modified), gray (ignored), red (conflict)
     - [ ] Icon legend in status bar or tooltip

**** Repository Info in Status Bar
     - [ ] Detect if current directory is in a git repo
     - [ ] Show current branch name
     - [ ] Show ahead/behind count (up2 down3)
     - [ ] Show dirty/clean status indicator

**** Git Context Menu Actions
     - [ ] "Git Status" - show status in preview pane or dialog
     - [ ] "Git Add" - stage selected files
     - [ ] "Git Reset" - unstage selected files
     - [ ] "Git Diff" - show diff in preview pane
     - [ ] "Git Blame" - show blame for selected file
     - [ ] "Open in GitHub/GitLab" - if remote configured

**** Git History View
     - [ ] View commit history for current directory
     - [ ] View file history for selected file
     - [ ] Show commit message, author, date
     - [ ] Checkout previous versions (stretch goal)

**** .gitignore Handling
     - [ ] Visual indication of ignored files (dimmed or hidden)
     - [ ] Toggle to show/hide ignored files
     - [ ] Quick add to .gitignore from context menu
...
** DONE Browser Tabs
   ...
   - [X] Ctrl+T / Ctrl+W keyboard shortcuts
   - [X] Ctrl+H / Ctrl+L for tab navigation (vim-style)
   - [X] Ctrl+Shift+1-6 for direct tab switching
   - [ ] Drag tabs to reorder (not yet implemented)

** DONE Customizable Keyboard Shortcuts
   CLOSED: [2024-12-01]
   - [X] Configurable via config.json hotkeys section
   - [X] Support for Ctrl, Shift, Alt, Cmd modifiers
   - [X] File operations: Copy, Cut, Paste, Delete, Rename, NewFile, NewFolder, SelectAll
   - [X] Navigation: Back, Forward, Up, Home, Refresh
   - [X] UI: FocusSearch (Ctrl+F), TogglePreview, ToggleHidden, Escape
   - [X] Tabs: NewTab, NewTabHome, CloseTab, NextTab, PrevTab, Tab1-6
   - [X] Platform-specific defaults (macOS uses Cmd for nav, Windows/Linux use Alt)
   - [ ] Settings UI to rebind shortcuts (future)
   - [ ] Import/export shortcut configs (future)
   - [ ] Preset profiles (Default, Vim, Emacs) (future)

** DONE File Conflict Resolution
   CLOSED: [2024-11-30]
   - [X] Dialog for copy/paste conflicts
   - [X] Options: Replace, Keep Both, Skip, Stop
   - [X] Apply to All checkbox for batch operations
   - [X] Shows file sizes and modification times
...
** TODO Update Documentation for New Features
   - [ ] Document Tab key for expand/collapse directories in tree view
   - [ ] Document shift+click multi-select in grid view
   - [ ] Document letter key cycling for quick-jump navigation
   - [ ] Update hotkeys modal/help screen with new shortcuts
   - [ ] Ensure keyboard shortcuts UI reflects all available shortcuts
