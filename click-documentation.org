...
| Priority | Handler                | Location              | PassOp | Consumes Clicks      |
|----------+------------------------+-----------------------+--------+----------------------|
| 1 (top)  | Global mouse tracker   | layout.go:40-44       | YES    | NO (observes only)   |
| 2        | File row clickables    | renderer.go:1519-1523 | NO     | YES (both L/R)       |
| 3        | Favorite row clickables| renderer.go:1642-1650 | NO     | YES (both L/R)       |
| 4        | Sidebar buttons        | various               | NO     | YES (left-click)     |
| 5        | bgClick (left-click)   | layout.go:113-128     | NO     | YES (left-click)     |
| 6        | bgRightClickTag        | layout.go:1314-1316   | NO     | YES (both L/R)       |
...
**** Event Flow for Left-Clicks

***** Scenario 4: Left-click on a file row
#+BEGIN_EXAMPLE
1. Global mouse tracker receives event (PassOp=YES)
   -> Records mousePos, sets lastClickModifiers
   -> Sets bgLeftClickPending = true (tentatively)
   -> Event passes through

2. File row handler receives event (PassOp=NO)
   -> Detects ButtonPrimary
   -> Sets rowLeftClicked = true
   -> Clears bgLeftClickPending
   -> Processes selection/double-click logic
   -> Event consumed (stops here)

3. After row processing in layoutFileList:
   -> Checks: if bgLeftClickPending && !rowLeftClicked
   -> Since rowLeftClicked=true, no background action taken
#+END_EXAMPLE

***** Scenario 5: Left-click on empty space (background) - DISMISSES CONTEXT MENU
#+BEGIN_EXAMPLE
1. Global mouse tracker receives event (PassOp=YES)
   -> Records mousePos, sets lastClickModifiers
   -> Sets bgLeftClickPending = true
   -> Event passes through

2. No file row handler exists at click position
   -> Event continues to bgRightClickTag handler (handles both L/R)

3. After row processing in layoutFileList:
   -> Checks: if bgLeftClickPending && !rowLeftClicked
   -> Since rowLeftClicked=false, calls r.onLeftClick()
   -> This dismisses: context menu, file menu, path edit mode
#+END_EXAMPLE

**** The Pending Flag Pattern

The ~bgRightClickPending~ and ~bgLeftClickPending~ flags serve as coordination mechanisms:

#+BEGIN_SRC go
// Set tentatively by global handler (for ALL clicks)
if e.Buttons.Contain(pointer.ButtonSecondary) {
    r.bgRightClickPending = true
    r.bgRightClickPos = r.mousePos
...
if r.bgRightClickPending && !rowRightClicked {
    // This was truly a background right-click - show menu
    r.menuVisible = true
...
if r.bgLeftClickPending && !rowLeftClicked {
    // This was truly a background left-click - dismiss menus
    r.onLeftClick()
}
...
**** Current Implementation (layout.go:46-73)
...
**** Renderer Fields for Background Click Tracking

#+BEGIN_SRC go
// renderer.go - Background click pending flags
bgRightClickPending bool        // Right-click detected, waiting to see if row claims it
bgRightClickPos     image.Point // Position of background right-click (for menu placement)
...
3. Left-click in the empty space - should dismiss context menu if shown, clear selection
4. Navigate to an empty directory
5. Right-click anywhere - should show background context menu
6. Left-click anywhere - should dismiss any open menus

** File Row Click Tests

1. Left-click on a file - should select it
2. Double-click on a folder - should navigate into it
3. Double-click on a file - should open it with default application
4. Right-click on a file - should show file context menu (Open, Copy, Cut, Delete, Rename)
5. Right-click on a folder - should show folder context menu (includes "Open in New Tab", "Add to Favorites")
6. Shift+click on multiple files - should enter multi-select mode with checkboxes

** Menu Dismissal Tests

1. Open context menu via right-click on file
2. Left-click on empty space - menu should dismiss
3. Open context menu via right-click on empty space
4. Left-click on a file row - menu should dismiss and file should be selected
5. Press Escape - menu should dismiss
